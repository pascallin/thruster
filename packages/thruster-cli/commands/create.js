...
async function loadGitTemplate(url) {
  console.log(
    "temp directory:",
    chalk.yellow(path.join(os.tmpdir(), "thruster", projectName))
  );
  ...
  download(url, path.join(os.tmpdir(), "thruster", projectName), (err) => {
    if (err) {
      ...
      console.log(
        chalk.red("Download failed. Please confirm your git repository URL.")
      );
      spinner.fail();
      return;
    }
...
async function loadNpmTemplate(npmPackageName) {
  const spinner = ora(
    `start installing template package(${npmPackageName})...`
  ).start();
